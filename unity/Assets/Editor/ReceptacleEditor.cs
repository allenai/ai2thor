...
public class ReceptacleEditor : Editor
{
    public override void OnInspectorGUI()
    {
        ...
        if (r.VisibilityCollider == null)
        {
            ...
            EditorGUILayout.TextArea(
                "You must define a visibility collider.\n"
                    + "If the item does not close, this should be the main collider.\n"
                    + "If the item closes, it should be a collider that is obscured when it is closed.",
        ...
        {
            r.VisibilityCollider = (Collider)
                EditorGUILayout.ObjectField(r.VisibilityCollider, typeof(Collider), false);
        }
    ...
    void CheckForClearAndDelete(Receptacle r, int index, ref int deletedPivotIndex)
    {
        ...
        Rect deleteArea = GUILayoutUtility.GetRect(100.0f, 25.0f);
        ...
        if (GUI.Button(deleteArea, "Delete pivot"))
        {
            if (
                EditorUtility.DisplayDialog(
                    "Delete pivot",
                    "Are you sure you want to delete pivot?",
                    "Yes",
                    "Cancel"
                )
            )
        ...
        Rect removeArea = GUILayoutUtility.GetRect(100.0f, 25.0f);
    ...
    void CheckForDragDropColliders(Receptacle r)
    {
        ...
        Rect dropArea = GUILayoutUtility.GetRect(100.0f, 100.0f);
    ...
    void TryToSetVisibilityCollider(Receptacle r, UnityEngine.Object potentialCollider)
    {
        ...
        {
            Debug.Log(
                "Visibility collider must be the receptacle or a child of the receptacle, not adding"
            );
        }
    ...
    void CheckForDragDropItems(Receptacle r, Transform pivot, string pivotDesc, string itemDesc)
    {
        ...
        EditorGUILayout.BeginVertical();
        Rect labelArea = GUILayoutUtility.GetRect(100.0f, 20.0f);
