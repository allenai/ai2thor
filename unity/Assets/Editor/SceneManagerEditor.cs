...
public class SceneManagerEditor : Editor
{
    ...
    public override void OnInspectorGUI()
    {
        ...
        sm.LocalPhysicsMode = (ScenePhysicsMode)
            EditorGUILayout.EnumPopup("Physics Mode", sm.LocalPhysicsMode);
        sm.AnimationMode = (SceneAnimationMode)
            EditorGUILayout.EnumPopup("Animation Mode", sm.AnimationMode);
        ...
        sm.FPSControllerPrefab = (GameObject)
            EditorGUILayout.ObjectField(
                "FPS controller prefab:",
                sm.FPSControllerPrefab,
                typeof(GameObject),
                false
            );
        ...
        if (ShowRequiredTypeCheckResult)
        {
            if (TypesNotFound != null && TypesNotFound.Length > 0)
            {
                ...
                EditorGUILayout.LabelField(
                    "The following required types were NOT found:",
                    EditorStyles.boldLabel
                );
                for (int i = 0; i < TypesNotFound.Length; i++)
                {
                    EditorGUILayout.LabelField(
                        " - " + TypesNotFound[i].ToString(),
        ...
        if (showBuildOptions)
        {
            ...
            sceneSelection = EditorGUILayout.Popup(
                "Scene to build",
                sceneSelection,
            ...
            {
                outputPath = EditorUtility.OpenFolderPanel(
                    "Scene output path",
                    outputPath,
                    string.Empty
                );
            ...
            foreach (BuildTarget bt in Targets)
            {
                if (GUILayout.Button("BUILD -> " + bt.ToString()))
                {
                    if (string.IsNullOrEmpty(outputPath) || !System.IO.Directory.Exists(outputPath))
                    {
                        EditorUtility.DisplayDialog(
                            "Invalid output path!",
                            "Select a valid scene output path",
                            "OK"
                        );
                    }
                    else
