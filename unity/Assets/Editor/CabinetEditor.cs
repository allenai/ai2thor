    ...
    public override void OnInspectorGUI()
    {
        ...
        switch (c.OpenStyle)
        {
            case CabinetOpenStyle.DoubleDoors:
                c.LeftDoor = (Transform)
                    EditorGUILayout.ObjectField("Left door", c.LeftDoor, typeof(Transform), true);
                c.RightDoor = (Transform)
                    EditorGUILayout.ObjectField("Right door", c.RightDoor, typeof(Transform), true);
                c.OpenAngleLeft = EditorGUILayout.Vector3Field(
                    "Open Angle (Left)",
                    c.OpenAngleLeft
                );
                c.ClosedAngleLeft = EditorGUILayout.Vector3Field(
                    "Closed Angle (Left)",
                    c.ClosedAngleLeft
                );
                c.OpenAngleRight = EditorGUILayout.Vector3Field(
                    "Open Angle (Right)",
                    c.OpenAngleRight
                );
                c.ClosedAngleRight = EditorGUILayout.Vector3Field(
                    "Closed Angle (Right)",
                    c.ClosedAngleRight
                );
                break;
            ...
            case CabinetOpenStyle.Drawer:
                c.DrawerDoor = (Transform)
                    EditorGUILayout.ObjectField("Drawer", c.DrawerDoor, typeof(Transform), true);
                c.OpenLocalPosition = EditorGUILayout.Vector3Field(
                    "Open Position (local)",
                    c.OpenLocalPosition
                );
                c.ClosedLocalPosition = EditorGUILayout.Vector3Field(
                    "Closed Position (local)",
                    c.ClosedLocalPosition
                );

                // visibility collider needs to change scale and position if drawer is open or closed
                c.VisCollider = (Transform)
                ...
                c.OpenVisColPosition = EditorGUILayout.Vector3Field(
                    "Visibility Collider Open Position (local)",
                    c.OpenVisColPosition
                );
                c.OpenVisColScale = EditorGUILayout.Vector3Field(
                    "Visibility Collider Open Scale (local)",
                    c.OpenVisColScale
                );
                // closed position and scale for visibility collider, set this manually in editor
                c.ClosedVisColPosition = EditorGUILayout.Vector3Field(
                    "Visibility Collider Closed Position (local)",
                    c.ClosedVisColPosition
                );
                c.ClosedVisColScale = EditorGUILayout.Vector3Field(
                    "Visibility Collider Closed Scale (local)",
                    c.ClosedVisColScale
                );
                break;
        }
            ...
            try
            {
                if (cabinet.ParentObj != null)
                {
                    ...
                    pivot.transform.position =
                        vCollider.bounds.center + (Vector3.down * vCollider.bounds.extents.y);
                }
                //}
            }
            catch (Exception e)
