...
public class SimObjectEditor : Editor
{
    void OnSceneGUI()
    {
        ...
        if (SimUtil.ShowIDs)
        {
            ...
            Handles.Label(
                (simObj.transform.position + Vector3.up * 0.1f),
                simObj.Type.ToString() + " : " + simObj.ObjectID.ToString(),
    ...
    public override void OnInspectorGUI()
    {
        ...
        if (!EditorUtility.IsPersistent(simObj))
        {
            if (!SimUtil.IsObjectIDValid(simObj.ObjectID))
            {
                ...
                EditorGUILayout.LabelField(
                    "This object has no Object ID. Use SceneManager to gather / generate SimObj IDs."
                );
            }
            else
            {
        ...
        simObj.Manipulation = (SimObjManipType)
            EditorGUILayout.EnumPopup("Manipulation type:", simObj.Manipulation);
        simObj.UseCustomBounds = EditorGUILayout.Toggle(
            "Use custom bounds",
            simObj.UseCustomBounds
        );
        if (!EditorUtility.IsPersistent(simObj))
        {
        ...
        if (simObj.UseCustomBounds)
        {
            ...
            simObj.BoundsTransform = (Transform)
                EditorGUILayout.ObjectField(simObj.BoundsTransform, typeof(Transform), true);
        ...
        if (!Application.isPlaying && !EditorUtility.IsPersistent(simObj))
        {
            ...
            bool showCustomBounds = GUILayout.Toggle(
                SimUtil.ShowCustomBounds,
                "Show custom bounds"
            );
            bool showObjectVisibility = GUILayout.Toggle(
                SimUtil.ShowObjectVisibility,
            ...
            if (
                showBasePivots != SimUtil.ShowBasePivots
