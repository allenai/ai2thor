    ...
    public class TestBaseProcedural : TestBase
    {
        ...
        protected string getTestResourcesPath(string filename = "", bool resourcesRelative = true)
        {
            ...
            var path =
                ...
                + string.Join(
                    "/",
                    NUnit.Framework.TestContext.CurrentContext.Test.FullName.Split('.')
                );
            return string.IsNullOrEmpty(filename) ? path : $"{path}/{filename}";
        }

        protected string getBuildMachineTestOutputPath(
            string filename = "",
            bool resourcesRelative = true
        )
        {
            ...
            // }
            var filenamePrefix = string.Join(
                "_",
                NUnit.Framework.TestContext.CurrentContext.Test.FullName.Split('.')
            );
