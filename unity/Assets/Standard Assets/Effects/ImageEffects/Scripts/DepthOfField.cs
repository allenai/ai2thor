...
namespace UnityStandardAssets.ImageEffects
{
    ...
    public class DepthOfField : PostEffectsBase
    {
        ...
        public override bool CheckResources()
        {
            ...
            {
                dx11bokehMaterial = CheckShaderAndCreateMaterial(
                    dx11BokehShader,
                    dx11bokehMaterial
                );
        ...
        float FocalDistance01(float worldDist)
        {
            return cachedCamera
                    .WorldToViewportPoint(
                        (worldDist - cachedCamera.nearClipPlane) * cachedCamera.transform.forward
                            + cachedCamera.transform.position
                    )
                    .z / (cachedCamera.farClipPlane - cachedCamera.nearClipPlane);
        }
        ...
        void OnRenderImage(RenderTexture source, RenderTexture destination)
        {
            ...
            focalDistance01 =
                (focalTransform)
                    ? (cachedCamera.WorldToViewportPoint(focalTransform.position)).z
                        / (cachedCamera.farClipPlane)
            ...
            else if ((blurType == BlurType.DX11) && dx11bokehMaterial)
            {
                ...
                if (highResolution)
                {
                    ...
                    rtLow = RenderTexture.GetTemporary(
                        source.width,
                        source.height,
                        0,
                        source.format
                    ...
                    var dest2 = RenderTexture.GetTemporary(
                        source.width,
                        source.height,
                        0,
                        source.format
                    ...
                    rtSuperLow1 = RenderTexture.GetTemporary(
                        source.width >> 1,
                        source.height >> 1,
                        0,
                        source.format
                    );
                    rtSuperLow2 = RenderTexture.GetTemporary(
                        source.width >> 1,
                        source.height >> 1,
                        0,
                        source.format
                    ...
                    dx11bokehMaterial.SetVector(
                        "_BokehParams",
                        new Vector4(
                            dx11BokehScale,
                            dx11BokehIntensity,
                            Mathf.Clamp(dx11BokehThreshold, 0.005f, 4.0f),
                            internalBlurWidth
                        )
                    );
                ...
                else
                {
                    ...
                    rtLow2 = RenderTexture.GetTemporary(
                        source.width >> 1,
                        source.height >> 1,
                        0,
                        source.format
                    ...
                    {
                        // capture fg coc
                        rtLow3 = RenderTexture.GetTemporary(
                            source.width >> 1,
                            source.height >> 1,
                            0,
                            source.format
                    ...
                    dx11bokehMaterial.SetVector(
                        "_Screen",
                        new Vector3(
                            1.0f / (1.0f * rtLow2.width),
                            1.0f / (1.0f * rtLow2.height),
                            internalBlurWidth
                        )
                    );
                    ...
                    dofHdrMaterial.SetVector(
                        "_Offsets",
                        ((1.0f * source.width) / (1.0f * rtLow2.width))
                            * internalBlurWidth
                            * Vector4.one
                    );
            ...
            else
            {
                ...
                int blurPass =
                    (
                        ...
                        || blurSampleCount == BlurSampleCount.Medium
                    )
                        ? 17
                        : 11;
