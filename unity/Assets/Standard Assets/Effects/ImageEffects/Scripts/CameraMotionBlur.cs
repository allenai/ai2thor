    ...
    public class CameraMotionBlur : PostEffectsBase
    {
        ...
        private void CalculateViewProjection()
        {
            ...
            if (_camera.stereoEnabled)
            {
                ...
                {
                    Matrix4x4 stereoViewMat = _camera.GetStereoViewMatrix(
                        eye == 0 ? Camera.StereoscopicEye.Left : Camera.StereoscopicEye.Right
                    );
                    Matrix4x4 stereoProjMat = _camera.GetStereoProjectionMatrix(
                        eye == 0 ? Camera.StereoscopicEye.Left : Camera.StereoscopicEye.Right
                    );
        ...
        public override bool CheckResources()
        {
            ...
            {
                dx11MotionBlurMaterial = CheckShaderAndCreateMaterial(
                    dx11MotionBlurShader,
                    dx11MotionBlurMaterial
                );
            }
        ...
        void OnRenderImage(RenderTexture source, RenderTexture destination)
        {
            ...
            // auto DX11 fallback!
            bool fallbackFromDX11 =
                filterType == MotionBlurFilter.ReconstructionDX11 && dx11MotionBlurMaterial == null;
            ...
            RenderTexture neighbourMax = RenderTexture.GetTemporary(
                tileWidth,
                tileHeight,
                0,
                rtFormat
            ...
            if (_camera.stereoEnabled)
            {
                ...
                motionBlurMaterial.SetMatrix(
                    "_StereoToPrevViewProjCombined1",
            ...
            if (preview)
            {
                ...
                motionBlurMaterial.SetMatrix("_PrevViewProj", prevViewProjMat);
                motionBlurMaterial.SetMatrix(
                    "_ToPrevViewProjCombined",
                    prevViewProjMat * invViewPrj
                );
            }
            ...
            if (filterType == MotionBlurFilter.CameraMotion)
            {
                ...
                // pitch (vertical)
                farHeur =
                    (Vector3.Angle(transform.up, prevFrameUp) / _camera.fieldOfView)
                    * (source.width * 0.75f);
                ...
                farHeur =
                    (Vector3.Angle(transform.forward, prevFrameForward) / _camera.fieldOfView)
                    * (source.width * 0.75f);
                ...
                if (distMag > Mathf.Epsilon && movementScale > Mathf.Epsilon)
                {
                    ...
                    blurVector.w =
                        movementScale
                        * (Vector3.Dot(transform.forward, distanceVector))
                        * (source.width * 0.5f);
                    // jump (maybe scale down further)
                    blurVector.x +=
                        movementScale
                        * (Vector3.Dot(transform.up, distanceVector))
                        * (source.width * 0.5f);
                    // strafe (maybe scale down further)
                    blurVector.y +=
                        movementScale
                        * (Vector3.Dot(transform.right, distanceVector))
                        * (source.width * 0.5f);
                }

                if (preview) // crude approximation
                    motionBlurMaterial.SetVector(
                        "_BlurDirectionPacked",
                        new Vector4(previewScale.y, previewScale.x, 0.0f, previewScale.z)
                            * 0.5f
                            * _camera.fieldOfView
                    );
                else
            ...
            else
            {
                ...
                if (
                    cam
                    && excludeLayers.value != 0
                    && replacementClear
                    && replacementClear.isSupported
                )
                {
            ...
            else
            {
                if (filterType == MotionBlurFilter.ReconstructionDX11 && !fallbackFromDX11)
                {
                    ...
                    dx11MotionBlurMaterial.SetFloat(
                        "_SoftZDistance",
                        Mathf.Max(0.00025f, softZDistance)
                    );
