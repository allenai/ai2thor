...
public class ResourceAssetManager {
    ...
    private Dictionary<string, List<ResourceAsset>> labelIndex = new Dictionary<string, List<ResourceAsset>>();
    ...
    public void RefreshCatalog() {
        ...
        foreach (string guid in this.findResourceGuids()) {
            ...
            if (assetPath.ToLower().EndsWith(".fbx") && AssetDatabase.LoadAllAssetsAtPath(assetPath).Length > 1) {
                continue;
            }
    ...
    private ResourceAssetCatalog readCatalog() {
        ...
        if (Path.HasExtension(catalogResourcePath)) {
            ...
            catalogResourcePath = catalogResourcePath.Remove(catalogResourcePath.Length - ext.Length, ext.Length);
        }
    ...
    public List<ResourceAssetReference<T>> FindResourceAssetReferences<T>(string label) where T : UnityEngine.Object {
        ...
        if (this.labelIndex.ContainsKey(label)) {
            foreach (var res in this.labelIndex[label]) {
                if (res.assetType == typeName) {
                    ...
                    ResourceAssetReference<T> assetRef = new ResourceAssetReference<T>(resourcePath: resourcePath, name: res.name);
