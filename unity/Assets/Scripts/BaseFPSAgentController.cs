...
namespace UnityStandardAssets.Characters.FirstPerson {
    ...
    abstract public class BaseFPSAgentController : MonoBehaviour {
        ...
        public void GetMaterials() {
            ColorChanger colorChangeComponent = physicsSceneManager.GetComponent<ColorChanger>();
            actionFinished(true, colorChangeComponent.GetMaterials());
        }
        ...
        */
        ...
        public void CreateRoom(Wall[] walls, float wallHeight, string wallMaterialId, string floorMaterialId, string ceilingMaterialId = "", float wallThickness = 0.0f, string namePostFix = "") {
            var wallPoints = walls.SelectMany(
            wall => new List<Vector3>() { wall.p0, wall.p1 }
            );
            ...
            // 	}
            // 	if (wall.materialId == "") {
            // 		wall.materialId = wallMaterialId;
            // 	}
            ...
            var room = RectangleRoom.roomFromWalls(
            walls.Select(wall => new Wall() {
                p0 = wall.p0,
                p1 = wall.p1,
                height = wallHeight,
        ...
        public void CreateHouse(House house) {
            ...
            room => RectangleRoom.roomFromWalls(
            room.walls,
            room.floor.materialId
            )
            );
            ...
            r => r.walls.Select(w => w.materialId).Concat(new List<string> { r.floor.materialId }))
            .Concat(new List<string> { house.ceilingMaterialId })
            );
