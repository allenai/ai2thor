...
public class InstantiatePrefabTest : MonoBehaviour {
    ...
    public bool PlaceObject(
        ...
        foreach (RotationAndDistanceValues quat in ToCheck) {
            ...
            if (CheckSpawnArea(sop, rsp.Point + rsp.ParentSimObjPhys.transform.up * (quat.distance + yoffset), quat.rotation, false)) {
                ...
                // attempt to sort corners so that first four corners are the corners closest to the spawn point we are checking against
                ...
                // don't spawn stuff half on a table and it falls over
    ...
    public Collider ColliderHitByObjectInSpawnArea(
        ...
        // move it into place so the bounding box is in the right spot to generate the overlap box later
