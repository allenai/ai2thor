...
public class ArticulatedAgentSolver : MonoBehaviour, MovableContinuous {
    ...
    private float accelerationDistance,
        beginDecelerationSpeed,
        decelerationDistance,
        beginDecelerationTime;
    private bool beginDeceleration,
        maxSpeed;
    float deceleration,
        speedupTime;
    ...
    public void ContinuousUpdate(float fixedDeltaTime) {
        ...
        Debug.Log(
            $"Damping force X equals: {dampingForceX} == clamp(-200 * {agentOrientedVelocity.x} * {currentAgentMoveParams.agentMass}, 100, 100)"
        );
        ...
        if (currentAgentMoveParams.agentState == ABAgentState.Moving) {
            ...
            if (distanceTransformedSoFar < accelerationDistance) {
                float desiredDistance =
                    ...
                    * Mathf.Pow(currentAgentMoveParams.timePassed + fixedDeltaTime, 2);
                ...
                float relativeForce =
                    forceScaler
                    * distanceDelta
                    * currentAgentMoveParams.agentMass
                    * currentAgentMoveParams.direction;

                relativeForce = Mathf.Clamp(
                    relativeForce,
                ...
                float desiredSpeed =
                    beginDecelerationSpeed * (remainingDistance / accelerationDistance);
            ...
            distanceTransformedThisFixedUpdate = Mathf.Clamp(
                Vector3.Dot(
                    ...
                    currentForward * currentAgentMoveParams.direction
                ),
                0,
                Mathf.Infinity
            );
            ...
            // above)
            float dampingForceZ = Mathf.Clamp(
                -100f * agentOrientedVelocity.z * currentAgentMoveParams.agentMass,
                -50f,
                50f
            );
            myAB.AddRelativeForce(new Vector3(0f, 0f, dampingForceZ));
            Debug.Log(
                $"Damping force Z equals: {dampingForceZ} == clamp(-100 * {agentOrientedVelocity.z} * {currentAgentMoveParams.agentMass}, -50, 50)"
            );
            ...
            if (distanceTransformedSoFar < accelerationDistance) {
                ...
                float relativeTorque = Mathf.Clamp(
                    forceScaler
                        * angularDistanceDelta
                        * myAB.inertiaTensor.y
                        * currentAgentMoveParams.direction,
                    -currentAgentMoveParams.maxForce,
                    currentAgentMoveParams.maxForce
                );
            ...
            distanceTransformedThisFixedUpdate =
                Mathf.Deg2Rad
                * Mathf.Clamp(
                    (currentRotation.y - prevStepTransformation.y)
                        * currentAgentMoveParams.direction,
                    0,
                    Mathf.Infinity
                );
