...
public class ProceduralRoomEditor : MonoBehaviour {
    ...
    private List<Vector3> getPolygonFromWallObject(
        GameObject wall,
        bool reverse = false,
        bool debug = false
    ) {
        ...
        if (debug) {
            Debug.Log(
                " wall "
                    ...
                    + wall.GetComponentInParent<SimObjPhysics>().gameObject.name
                    + " box "
                    + box.center
                    + " offset "
                    + offset
                    ...
                    + box.size
                    + " p0 "
                    + wall.transform.TransformPoint(box.center - offset).ToString("F8")
                    + " p1 "
                    + wall.transform.TransformPoint(
                            box.center + new Vector3(offset.x, -offset.y, 0.0f)
                        )
                        .ToString("F8")
                    + $"local p0: {(box.center + new Vector3(offset.x, -offset.y, 0.0f)).ToString("F8")} p1: {(box.center + new Vector3(offset.x, -offset.y, 0.0f)).ToString("F8")}"
            );
        }
    ...
    private IEnumerable<ConnectionAndWalls> serializeConnections(
        IEnumerable<SimObjPhysics> connections,
        SimObjType filterType,
        string prefix,
        ...
        var connectionsWithWalls = connections
            ...
            .Select(
                (d, i) => {
                    ...
                    Debug.Log(
                        $"----- i: {i},  {prefix} {d.gameObject.name} p0 {poly[0]} p1 {poly[1]}"
                    );
                    var polyRev = getPolygonFromWallObject(d.BoundingBox, !flippedForward);
                    ConnectionProperties connectionProps =
                        d.GetComponentInChildren<ConnectionProperties>();
                    var materialId = "";
                    ...
                    wallColliders = wallColliders
                        .Where(s => s.GetComponent<SimObjPhysics>()?.Type == SimObjType.Wall)
                        .ToArray();
                    ...
                    bool wallPointOnConnectionLine(
                        Vector3 wallP0,
                        Vector3 wallP1,
                        Vector3 direction,
                        Vector3 wallNormal,
                        ...
                        var t0 = new Vector3(
                            connP0ToWallP1.x / direction.x,
                            connP0ToWallP1.y / direction.y,
                            connP0ToWallP1.z / direction.z
                        );
                        // var proy1 = Vector3.Dot(connP1ToWallP0, -direction) * -direction;
                        var t1 = new Vector3(
                            connP1ToWallP0.x / -direction.x,
                            connP1ToWallP0.y / -direction.y,
                            connP1ToWallP0.z / -direction.z
                        );
                        ...
                        if (
                            ...
                            Debug.Log(
                                $"************* wall {name}, wallp0 ({wallP0.x}, {wallP0.y}, {wallP0.z}),  wallp1 ({wallP1.x}, {wallP1.y}, {wallP1.z}), connP0 {p0Ref},  connP1 {p1Ref}, connP0ToWallP1 {connP0ToWallP1}, connP1ToWallP0 {connP1ToWallP0} dir {direction} connP1ToWallP0 {connP1ToWallP0} connP0ToWallP1 {connP0ToWallP1} t0.x {t0.x}, t0.y {t0.y}, t0.z {t0.z} t1.x {t1.x}, t1.y {t1.y}, t1.z {t1.z} onLine0 {onLine0}, onLine1 {onLine1} dot0 {dot0} dot1 {dot1}  num0.z {connP0ToWallP1.z} num1.z {connP1ToWallP0.z} dir.z {direction.z}"
                            );
                        }
                        ...
                        return (
                                ...
                                && (
                                    (
                                        Math.Abs(direction.x) > dirEps
                                        && t0.x <= (1.0f + tEpsStrict)
                                        && t0.x >= (0.0f - tEpsStrict)
                                    )
                                    || (
                                        (
                                            Math.Abs(direction.z) > dirEps
                                            && t0.z <= (1.0f + tEpsStrict)
                                            && t0.z >= (0.0f - tEpsStrict)
                                        )
                                    )
                                )
                            )
                            || (
                                ...
                                && (
                                    (
                                        Math.Abs(direction.x) > dirEps
                                        && t1.x <= (1.0f + tEpsStrict)
                                        && t1.x >= (0.0f - tEpsStrict)
                                    )
                                    || (
                                        (
                                            Math.Abs(direction.z) > dirEps
                                            && t1.z <= (1.0f + tEpsStrict)
                                            && t1.z >= (0.0f - tEpsStrict)
                                        )
                                    )
                                )
                            );
                    ...
                    bool pointOnWallLine(
                        ...
                        var t = new Vector3(
                            originToP.x / direction.x,
                            originToP.y / direction.y,
                            originToP.z / direction.z
                        );
                        // var proyLength = Vector3.Dot(num, direction);
                    ...
                    var connectionNormal = flippedForward
                        ? -d.transform.forward
                        : d.transform.forward;
                    ...
                    var wallRight = colliderDistances.Aggregate(
                        ...
                        (min, next) =>
                            ...
                            && Vector3.Dot(next.collider.transform.forward, connectionNormal)
                                >= normalEps
                            && !pointOnWallLine(
                                next.p0,
                                dir,
                                p0World,
                                next.collider.gameObject.name,
                                "right"
                            )
                                ? next
                                : min
                    ...
                    var toDelete = colliderDistances.Where(next =>
                        (
                            Vector3.Dot(next.collider.transform.forward, connectionNormal)
                                >= normalEps
                            && pointOnWallLine(
                    ...
                    Debug.Log(
                        $"&&&&&&&&&& TODELETE {d.gameObject.name} "
                            + string.Join(", ", toDelete.Select(w => w.collider.gameObject.name))
                    );
                    ...
                    Debug.Log(
                        "walls_right "
                            + wallRight.collider.gameObject.name
                            + " dist "
                            + wallRight.p0SqrDistance
                            + " wall_left "
                            + wallLeft.collider.gameObject.name
                            + " dist "
                            + wallLeft.p1SqrDistance
                            + " backwallLeft "
                            + backWallClosestLeft.collider.gameObject.name
                            + " backwallRight "
                    ...
                    var material = backWallClosestRight
                        .collider.GetComponent<MeshRenderer>()
                        .sharedMaterial;
                    ...
                    var wallRev = new PolygonWall {
                        ...
                        material = new MaterialProperties() {
                            ...
                            tilingDivisorY =
                                backWallClosestRight.height / material.mainTextureScale.y
                        },
                    };
                    ...
                    Debug.Log(
                        $"^^^^^^^^^^^^ Created wall p0: {wall.polygon[0].ToString("F8")} p1: {wall.polygon[1].ToString("F8")}"
                    );
                    ...
                    var assetId = !string.IsNullOrEmpty(d.assetID)
                        ? d.assetID
                        : PrefabNameRevert.GetPrefabAssetName(d.gameObject);
                    ...
                    if (filterType == SimObjType.Doorway) {
                        connection = new Thor.Procedural.Data.Door {
                            ...
                            {
                                new Vector3(xLen, 0.0f, box.size.z / 2.0f),
                                new Vector3(xLen + box.size.x, box.size.y, box.size.z / 2.0f)
                            },
                            ...
                            type = Enum.GetName(
                                ...
                                (connectionProps?.Type).GetValueOrDefault()
                            ),

                            openable = d.SecondaryProperties.Contains(
                    ...
                    var wallsToCreate = new List<(PolygonWall wall, string afterWallId)>()
                    {
                        (wall, wallLeft.collider.name),
                        (wallRev, backWallClosestLeft.collider.name)
                    };
                    ...
                    Debug.Log(
                        "^^^^^^^^^^^^^^^^ SUPPOSED TO ADD PIECE OF SHIT "
                            + string.Join(", ", wallsToCreate.Select(x => x.afterWallId))
                    );
    ...
    private PolygonWall createNewWall(
        string id,
        ConnectionProperties connectionProps,
        Vector3 p0,
        Vector3 p1,
    ...
    private ProceduralHouse regenerateWallsData(ProceduralHouse house) {
        ...
        var wallsGOs = root.GetComponentsInChildren<SimObjPhysics>()
            .Where(s => s.Type == SimObjType.Wall);
        ...
        var wallsJson = wallsGOs
            ...
            .ToList();
        var simObjs = GameObject
            .Find(ProceduralTools.DefaultObjectsRootName)
            .GetComponentsInChildren<SimObjPhysics>();

        var wallsDict = new Dictionary<string, PolygonWall>(
            house.walls.ToDictionary(w => w.id, w => w)
        );

        var connectionsDict = new Dictionary<string, WallRectangularHole>(
            house
        ...
        // }
        var doorWalls = serializeConnections(simObjs, SimObjType.Doorway, "door", wallsDict)
            .ToList();
        //doorWalls = doorWalls.Take(1).ToList();
        // var doorWalls = new List<ConnectionAndWalls>();

        var windowWalls = serializeConnections(simObjs, SimObjType.Window, "window", wallsDict)
            .ToList();

        Debug.Log(
            "+++++++++++++++ Windows "
                + string.Join(", ", windowWalls.SelectMany(x => x.walls).Select(w => w.wall.id))
        );
        ...
        var wallWithInsertLocation = allNewConnections
            ...
            .Select(wallIdPair => {
                ...
                return (
                    ...
                    index: wallsJson
                        .Select((w, i) => (wall: w, index: i))
                        .First(w => string.Equals(w.wall.id, wallIdPair.afterWallId))
                        .index
                );
            });
        ...
        Debug.Log(
            $"################# WALLS Before DELETE {string.Join(",", house.walls.Select(c => c.id))}"
        );
    ...
    public void SerializeSceneFromGameObjects(string outFilename) {
        ...
        Debug.Log(
            $"Serializing to: '{outFilename}', using procedural params and elements not in scene from: '{this.layoutJSONFilename}'"
        );
        ...
        house.objects = simObjects
            ...
            .Select(obj => {
                ...
                var serializedObj = new HouseObject() {
                    ...
                    room = room,
                    types = new List<Taxonomy>()
                    {
                        new Taxonomy()
                        {
                            name = Enum.GetName(typeof(SimObjType), obj.simObj.ObjType)
                        }
                    },
                    assetId = obj.assetId
                };
        ...
        var roomIdToProps = floorRoot
            .GetComponentsInChildren<RoomProperties>()
            .ToDictionary(
                rp => rp.GetComponentInParent<SimObjPhysics>().ObjectID,
                rp => new { roomProps = rp, simOb = rp.GetComponentInParent<SimObjPhysics>() }
            );
        ...
        var sceneLights = GameObject
            .Find(ProceduralTools.DefaultLightingRootName)
