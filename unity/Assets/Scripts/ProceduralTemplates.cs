...
namespace Thor.Procedural
{
    ...
    public static class Templates
    {
        ...
        )
        {
            ...
            Func<string, int[][]> toIntArray = (string l) =>
                l.Split('\n')
                    .Select(x =>
                        x.Split(' ')
                            .Select(i =>
                    ...
                    .Where(a => a.Length > 0)
                    .ToArray();
            Func<string, string[][]> toStringArray = (string l) =>
                l.Split('\n')
                    .Select(x =>
                        x.Split(' ').Where(x => !string.IsNullOrEmpty(x)).Select(i => i).ToArray()
                    )
                    .Where(a => a.Length > 0)
                    .ToArray();
            ...
            if (houseTemplate.doors != null)
            {
                holePairs = holePairs.Concat(
                    houseTemplate.doors.Select(d => new KeyValuePair<string, WallRectangularHole>(
                        d.Key,
                        d.Value as WallRectangularHole
                    ))
                );
            }
            ...
            Func<int, int, (int row, int column)[]> manhattanNeighbors = (int row, int column) =>
            ...
            var doorDict =
                new Dictionary<
                    (string id, int layer, (int row, int column) index),
                    HashSet<(int row, int column)>
                >();
                                ...
                                objectRow.Select(
                                    (id, column) =>
                                    {
                                        ...
                                        if (isHole)
                                        {
                                            ...
                                            {
                                                (row, column)
                                            }.Concat(manhattanNeighbors(row, column));
                                            var holeContinuation = isHole
                                                ? neighborIndices.Where(n =>
                                                    ProceduralTools.withinArrayBoundary(
                                                        n,
                                                        rows,
                                                        columns
                                                    )
                                                    && objectArray[layer][n.row][n.column] == id
                                                )
                                                : new List<(int, int)>();
                                            ...
                                            var neighborFunctions = neighborIndices
                                                ...
                                                .Where(v =>
                                                {
                                                    return ProceduralTools.withinArrayBoundary(
                                                        ...
                                                        && objectArray[layer][v.index.row][
                                                            v.index.column
                                                        ] == id; // id of door continues in neighbor
            ...
            var roomsWithWalls = roomIds
                ...
                .Select(intId =>
                {
                    ...
                    room.floorPolygon = walls2D
                        .Select(p => new Vector3(
                            (float)p.Item1.row,
                            template.floorYPosition,
                            (float)p.Item1.column
                        ))
                        .ToList();
                    ...
                    room.ceilings =
                        (room.ceilings != null && room.ceilings.Count > 0)
                            ? room.ceilings
                            : new List<Ceiling>() { new Ceiling() };

                    room.ceilings[0].material = ceilingMat;
                    room.ceilings[0].polygon = room
                        .floorPolygon.Select(p => new Vector3(p.x, p.y + wallHeight, p.z))
                        .ToList();
                            ...
                            (w, i) =>
                            {
                                ...
                                wallCopy.polygon = new List<Vector3>()
                                {
                                    new Vector3(
                                        (float)w.Item1.row,
                                        template.floorYPosition,
                                        (float)w.Item1.column
                                    ),
                                    new Vector3(
                                        (float)w.Item2.row,
                                        template.floorYPosition,
                                        (float)w.Item2.column
                                    ),
                                    new Vector3(
                                        (float)w.Item2.row,
                                        template.floorYPosition + wallHeight,
                                        (float)w.Item2.column
                                    ),
                                    new Vector3(
                                        (float)w.Item1.row,
            ...
            doorDict = doorDict
                .GroupBy(d => d.Value, new CustomHashSetEqualityComparer<(int, int)>())
                .ToDictionary(p => p.First().Key, p => p.Key); // .SelectMany(d => d.Select(x => x)).ToDictionary(p => p.Key, p => p.Value);
            ...
            var holes = doorDict
                .SelectMany(
                    (d, i) =>
                    {
                        ...
                        hole.room0 = roomIntToId(
                            room0Id,
                            houseTemplate
                                .rooms.GetValueOrDefault(room0Id.ToString(), defaultRoomTemplate)
                                .floorTemplate
                        );
                        hole.room1 = roomIntToId(
                            room1Id,
                            houseTemplate
                                .rooms.GetValueOrDefault(room1Id.ToString(), defaultRoomTemplate)
                                .floorTemplate
                        );
                        ...
                        var holeStartCoords = (
                            row: holeStart.row + holeAdjustOffset.row,
                        ...
                        var wall0 = room0Walls2D.Find(x =>
                        {
                            ...
                            var doorDirVec = new Vector2(
                                (float)doorDir.column,
                                (float)doorDir.row
                            ).normalized;
                            var dot = Vector2.Dot(wallDir, doorDirVec);
                            // TODO add eps
                            return (
                                    (xDiff == 0.0 && xDiff1 == 0.0)
                                    || (zDiff == 0.0 && zDiff1 == 0.0)
                                )
                                && Math.Abs(dot) < 1e-4;
                        });
                        ...
                        var minVector = new Vector3(
                            (float)(holeStartCoords.column - distToZeros.x),
                            (float)floorTemplate.floorYPosition,
                            0.0f
                        );
                        var maxVector = minVector + holeOffset.max;
                        hole.holePolygon = new List<Vector3> { minVector, maxVector };
                        var right = (
                        ...
                        hole.assetPosition =
                            wallSpace * (minVector + holeOffset.offset + (holeOffset.max / 2.0f));
            ...
            var houseObjects = objectCoordinates
                ...
                .SelectMany(item =>
                {
                    ...
                    if (template != null)
                    {
                        ...
                        obj.position =
                            new Vector3(
                                (float)coord.row - distToZeros.x + obj.position.x,
                                floorTemplate.floorYPosition + obj.position.y,
                                (float)coord.column - distToZeros.z + obj.position.z
                                ...
                                foreach (var w2_2 in wallMap[w2_1])
                                {
                                    if (
                                        (
                                            ...
                                            && w1_2.Item1 == w2_1.Item1
                                            && w2_1.Item1 == w2_2.Item1
                                        )
                                        || (
                                            w1_1.Item2 == w1_2.Item2
        ...
        )
        {
            return boundaryGroups.ToDictionary(
                ...
                bg => new HashSet<((double, double), (double, double))>(
                    bg.Value.Select(pair =>
                        (
                            (
                                Math.Round(pair.Item1.row * scale, precision),
                                Math.Round(pair.Item1.col * scale, precision)
                            ),
                            (
                                Math.Round(pair.Item2.row * scale, precision),
                                Math.Round(pair.Item2.col * scale, precision)
                            )
                        )
                    )
                )
        ...
        )
        {
            ...
            var output = new List<((double row, double col), (double row, double col))>()
            {
                walls[0]
            };
        ...
        public static Dictionary<
            int,
            List<((double row, double column), (double row, double column))>
        > getXZRoomToWallDict(
            Dictionary<
                (int, int),
                HashSet<((double row, double col), (double row, double col))>
        ...
        public static string holeTemplateIdToHouseId(
