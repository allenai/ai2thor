...
public class HoleMetadata : MonoBehaviour {
    ...
    void OnDrawGizmos() {
        ...
        var k = new List<Vector3>() { Vector3.zero, new Vector3(0, 0, size.z) };

        var corners = new List<List<Vector3>>
        {
            new List<Vector3>() { Vector3.zero, new Vector3(size.x, 0, 0) },
            new List<Vector3>() { Vector3.zero, new Vector3(0, size.y, 0) },
            new List<Vector3>() { Vector3.zero, new Vector3(0, 0, size.z) }
        }
            .CartesianProduct()
            .Select(x => x.Aggregate(bottomLeft, (acc, v) => acc + v))
            .ToList();
        ...
        var sides = new List<List<Vector3>>()
        {
            corners.Take(4).ToList(),
