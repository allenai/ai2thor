...
namespace Thor.Procedural {
    ...
    public static class ProceduralTools {
        ...
        public static Mesh GenerateFloorMesh(RectangleRoom room, float yOffset = 0.0f) {
            ...
            Vector2[] m_points = new Vector2[room.walls.Length];
            for (int i = 0; i < m_points.Length; i++) {
                m_points[i] = new Vector2(room.walls[i].p0.x, room.walls[i].p0.z);
                //Debug.Log("Point " + i + " is " + m_points[i]);
            }

            int[] triangleIndices = TriangulateVertices();
            //Debug.Log("TriangleIndices has length of " + triangleIndices.Length);

            // Get array of vertices for floor
            Vector3[] floorVertices = new Vector3[room.walls.Length];
            for (int i = 0; i < floorVertices.Length; i++) {
                floorVertices[i] = new Vector3(m_points[i].x, yOffset, m_points[i].y);
            }
