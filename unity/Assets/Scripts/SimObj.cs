...
public class SimObj : MonoBehaviour, SimpleSimObj {
    ...
    public static SimObjType[] OpenableTypes = new SimObjType[]
    {
        SimObjType.Fridge,
        SimObjType.Cabinet,
        SimObjType.Microwave,
        SimObjType.LightSwitch,
        SimObjType.Blinds,
        SimObjType.Book,
        SimObjType.Toilet
    };
    public static SimObjType[] ImmobileTypes = new SimObjType[]
    {
        SimObjType.Chair,
        SimObjType.Toaster,
        SimObjType.CoffeeMachine,
        SimObjType.Television,
        SimObjType.StoveKnob
    };
    private static Dictionary<SimObjType, Dictionary<string, int>> OPEN_CLOSE_STATES =
        new Dictionary<SimObjType, Dictionary<string, int>>
        {
            {
                SimObjType.Microwave,
                new Dictionary<string, int> { { "open", 2 }, { "close", 1 } }
            },
            {
                SimObjType.Laptop,
                new Dictionary<string, int> { { "open", 2 }, { "close", 1 } }
            ...
            {
                SimObjType.Book,
                new Dictionary<string, int> { { "open", 1 }, { "close", 2 } }
            },
            {
                SimObjType.Toilet,
                new Dictionary<string, int> { { "open", 2 }, { "close", 3 } }
            },
            {
                SimObjType.Sink,
                new Dictionary<string, int> { { "open", 2 }, { "close", 1 } }
        ...
        if (r != null) {
            ...
            if (UseWidthSearch) {
                topPoint =
                    ...
                    + (Vector3.forward * r.bounds.extents.z);
                bottomPoint =
                    centerPoint
            ...
            if (c != null) {
                ...
                if (UseWidthSearch) {
                    topPoint =
                        ...
                        + (Vector3.forward * c.bounds.extents.z);
                    bottomPoint =
                        centerPoint
    ...
    protected virtual void OnEnable() {
        ...
        switch (Manipulation) {
            ...
            case SimObjManipType.StaticNoPlacement:
                Manipulation =
                    (rearrangeable != null) ? SimObjManipType.Rearrangeable : Manipulation;
                break;
        }
            ...
            if (UseCustomBounds && SimUtil.ShowCustomBounds) {
                if (BoundsTransform != null) {
                    ...
                    Gizmos.color = Color.Lerp(
                        VisibleNow ? Color.yellow : Color.cyan,
                        Color.clear,
                        0.45f
                    );
