...
namespace Thor.Procedural
{
	public class RoomCreator : MonoBehaviour
	{
		...
		public void Start() {
			...
			// 	new int[]{1, 2, 2, 2, 2, 2, 1, 1},
			// 	new int[]{1, 2, 2, 2, 1, 1, 1, 1},
			// 	new int[]{1, 2, 2, 2, 1, 1, 1, 1},
			// 	new int[]{1, 1, 2, 2, 2, 2, 1, 1},
			// 	new int[]{1, 1, 2, 2, 2, 1, 1, 1},
			...
			// foreach (var entry in roomsToWalls) {
			// 	ProceduralTools.roomFromWallIndexDictionary(
			// 	"Floor",
			// 	new Dictionary<int, IEnumerable<(int row, int col)>>() {
			...
			// ProceduralTools.roomFromWallIndexDictionary(
			// 	"Floor",
			// 	roomsToWalls.Where(entry => entry.Key == 3).ToDictionary(m => m.Key, m => m.Value),
			// 	arrayMap2.Length,
			// 	arrayMap2[0].Length,
			// 	height,
			// 	wallHeight,
		...
		public static class ProceduralTools
		{
			...
			private static Vector3 vectorIntersectThickness2(Wall wall1, Wall wall2, bool toCreateFirstArg = true) {
				//return wall1.thickness;
				var wall1p0p1 = (wall1.p1 - wall1.p0).normalized;
				var wall2p0p1 = (wall2.p1 - wall2.p0).normalized;
				...
				if (Mathf.Abs(denominator) < 1E-4) {
					Debug.Log(" !!! den 0");
					var thickness = toCreateFirstArg ? wall1.thickness : wall2.thickness;
					var pseduoCrossSign = Mathf.Sign(normal1.x * normal2.z - normal1.z * normal2.x);
					pseduoCrossSign = toCreateFirstArg ? -pseduoCrossSign : pseduoCrossSign;
					return wall1p0 + wall1p0p1 * thickness * (pseduoCrossSign);
				}
				var t = Vector3.Dot(c, b) / denominator;
				Debug.Log(" --- den not zero " + Vector3.Magnitude((wall1p0 + t * (wall1p1 - wall1p0)) - wall1p1));
			...
			public static GameObject createAndJoinWall(
				...
				if (globalVertexPositions) {
					...
					//     -(width / 2.0f) * Vector3.right - new Vector3(0.0f, toCreate.height / 2.0f, 0.0f),
					// 	-(width / 2.0f) * Vector3.right + new Vector3(0.0f, toCreate.height / 2.0f, 0.0f),
					// 	(width / 2.0f) * Vector3.right + new Vector3(0.0f, toCreate.height / 2.0f, 0.0f),
					// 	(width / 2.0f) * Vector3.right - new Vector3(0.0f, toCreate.height / 2.0f, 0.0f)
					// 	};
			...
			public static GameObject createMultiRoomFloorGameObject(
				...
				// rooms.Select((room, index) => {
				// 	Debug.Log($"WALL CREATE {index}");
				// 	return ProceduralTools.createWalls(room, $"Structure_{index}");
				// }).Select(wallGO => {
				// 	floorGameObject.transform.parent = wallGO.transform;
				// 	return wallGO;
				// });
				Debug.Log($"After structure creation {rooms.Count()}");
				return floorGameObject;
			}
			...
			private static IEnumerable<(int row, int col)> traverseBoundary2(int[][] roomArray, (int row, int col) start, int target, int emptyValue) {
			...
			private static void traverseBoundary(int[][] roomArray, (int row, int col) current, IEnumerable<(int row, int col)> taboo, (int row, int col) start, int target, List<(int, int)> output, int emptyValue) {
				...
				if (start.col == current.col && start.row == current.row) {
					...
					//var value = roomArray[current.row][current.col];
					...
					Debug.Log($"current {current} neighb {string.Join("", neighbors)}");
					// (int row, int col) lastNeighborDelta = (0, 0);
					foreach (var neighborDelta in neighbors) {
						...
						if (
							...
							Debug.Log($"output, current {current} out {string.Join("", output)}");
							traverseBoundary(roomArray, prevNeighbor, new List<(int, int)>() { neighborDelta, (-neighborDelta.row, -neighborDelta.col) }, start, target, output, emptyValue);
							break;
						}
					}
				}
			...
			public static Dictionary<int, IEnumerable<(int row, int col)>> createRoomsFromGenerationArray(int[][] roomArray, int emptyValue = 1, Vector2? scale = null) {
				...
				var valueToIndices = distinct.ToDictionary(
					val => val,
					val =>
						 roomArray.Select((row, rowIndex) => (row: rowIndex, col: ArrayUtility.FindIndex(row, v => v == val)))
						.First(
							(index) => index.col != -1
						)
					);

				// var valueToIndices2 = distinct.ToDictionary(
				//     val => val, 
				//     val => roomArray.Select(
				//             (row, rowIndex) => r)

				//     roomArray.Select(
				//             (row, rowIndex) => (rowIndex, colIndex: ArrayUtility.FindIndex(row, v => v == val))
				//     ).Where(
				//             tuple => isWall(roomArray, tuple.rowIndex, tuple.colIndex)
				//     )
				// );

				var d = valueToIndices.Select(k => isWall(roomArray, (k.Value.row, k.Value.col)));

				Debug.Log($"s {string.Join(", ", d)}");

				var roomToSortedWalls = valueToIndices.ToDictionary(
						c => c.Key,
						c => {
							// var firstWall = c.Value;
							// Debug.Log($"room {c.Key} index {firstWall}");
							// var orderedWallPolygonPoints = new List<(int row, int col)>() { firstWall };
							// Debug.Log($"2 room {c.Key} index {firstWall}");
							// if (isWall(roomArray, (firstWall.row, firstWall.col + 1))) {
							// 	orderedWallPolygonPoints.Add((firstWall.row, firstWall.col + 1));
							// }
							//if (c.Key == 2)
							// traverseBoundary(roomArray, (firstWall.row, firstWall.col + 1), new List<(int, int)>() { (0, -1), (-1, 0) }, firstWall, c.Key, orderedWallPolygonPoints, emptyValue);
							return traverseBoundary2(roomArray, c.Value, c.Key, emptyValue);
							// return orderedWallPolygonPoints;
						}
				);

				// ArrayUtility.FindIndex(roomArray, )
			...
			public static GameObject roomFromWallIndexDictionary(
				...
				// createFloorGameObject(string name, RectangleRoom room, AssetMap < Material > materialDb, string simObjId, float receptacleHeight = 0.7f, float floorColliderThickness = 1.0f, Vector3 ? position = null)
		...
		//         private void g(int[][] roomArray) {
		//             List<(int, int)> wallIndices;
		//             var rows = roomArray.Length;
		//             for( var rowIndex = 1; rowIndex < rows; rowIndex++) {
		//                 var row = roomArray[rowIndex];
		//                 for (var colIndex = 1; colIndex < row.Length; colIndex++) {
		//                     if ( roomArray[rowIndex][colIndex] != roomArray[rowIndex - 1][colIndex]) {
		//                         wallIndices
		//                     }
		//            # there's a wall between (row, col) and (row - 1, col)
		//        if (row, col) != (row, col - 1):
		//            # there's a wall between (row, col) and (row, col - 1)
		//         }
