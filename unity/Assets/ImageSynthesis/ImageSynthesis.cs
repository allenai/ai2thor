...
//
    ...
    public byte[] Encode(string passName, RenderTextureFormat format = RenderTextureFormat.Default, int width = -1, int height = -1, bool jpg = false) 
    {
		...
		foreach (var pass in capturePasses)
            if(pass.name == passName)
                return Encode(pass.camera, width, height, pass.supportsAntialiasing, pass.needsRescale, jpg, format);
	...
	private byte[] Encode(Camera cam, int width, int height, bool supportsAntialiasing, bool needsRescale, bool jpg = false, RenderTextureFormat format = RenderTextureFormat.Default)
	{
		var mainCamera = GetComponent<Camera>();
		var depth = 24;
		var readWrite = format == RenderTextureFormat.RFloat ? RenderTextureReadWrite.Linear : RenderTextureReadWrite.Default;
