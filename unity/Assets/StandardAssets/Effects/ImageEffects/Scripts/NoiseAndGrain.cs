    ...
    public class NoiseAndGrain : PostEffectsBase
    {
...
#endif
                dx11NoiseMaterial = CheckShaderAndCreateMaterial(
                    dx11NoiseShader,
                    dx11NoiseMaterial
                );
            }
        ...
        void OnRenderImage(RenderTexture source, RenderTexture destination)
        {
            ...
            if (dx11Grain && supportDX11)
            {
                ...
                dx11NoiseMaterial.SetVector(
                    "_NoisePerChannel",
                    monochrome ? Vector3.one : intensities
                );
                dx11NoiseMaterial.SetVector(
                    "_MidGrey",
                ...
                if (softness > Mathf.Epsilon)
                {
                    RenderTexture rt = RenderTexture.GetTemporary(
                        (int)(source.width * (1.0f - softness)),
            ...
            else
            {
                ...
                noiseMaterial.SetVector(
                    "_NoiseTilingPerChannel",
                    monochrome ? Vector3.one * monochromeTiling : tiling
                );
                noiseMaterial.SetVector(
                    "_MidGrey",
                    new Vector3(midGrey, 1.0f / (1.0f - midGrey), -1.0f / midGrey)
        ...
        )
        {
            ...
            for (float x1 = 0.0f; x1 < 1.0f; x1 += stepSizeX)
            {
                for (float y1 = 0.0f; y1 < 1.0f; y1 += stepSizeY)
                {
                    ...
                    GL.MultiTexCoord2(
                        0,
                        tcXStart + texTile * texTileMod,
                        tcYStart + texTile * texTileMod
                    );
