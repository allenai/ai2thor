...
namespace UnityStandardAssets.ImageEffects
{
    ...
    public class SunShafts : PostEffectsBase
    {
        ...
        public override bool CheckResources()
        {
            ...
            simpleClearMaterial = CheckShaderAndCreateMaterial(
                simpleClearShader,
                simpleClearMaterial
            );
...
#else
                var format = GetComponent<Camera>().hdr
                    ? RenderTextureFormat.DefaultHDR
                    : RenderTextureFormat.Default;
#endif
                RenderTexture tmpBuffer = RenderTexture.GetTemporary(
                    source.width,
                    source.height,
                    0,
                    format
