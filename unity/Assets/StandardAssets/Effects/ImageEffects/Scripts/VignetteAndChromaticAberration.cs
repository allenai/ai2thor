...
namespace UnityStandardAssets.ImageEffects
{
    ...
    public class VignetteAndChromaticAberration : PostEffectsBase
    {
        ...
        public override bool CheckResources()
        {
            ...
            m_SeparableBlurMaterial = CheckShaderAndCreateMaterial(
                separableBlurShader,
                m_SeparableBlurMaterial
            );
            m_ChromAberrationMaterial = CheckShaderAndCreateMaterial(
        ...
        void OnRenderImage(RenderTexture source, RenderTexture destination)
        {
            ...
            if (doPrepass)
            {
                ...
                if (Mathf.Abs(blur) > 0.0f)
                {
                    ...
                    { // maybe make iteration count tweakable
                        m_SeparableBlurMaterial.SetVector(
                            "offsets",
                            new Vector4(0.0f, blurSpread * oneOverBaseSize, 0.0f, 0.0f)
                        );
                        RenderTexture color2B = RenderTexture.GetTemporary(
                            rtW / 2,
                            rtH / 2,
                            0,
                            source.format
            ...
            m_ChromAberrationMaterial.SetVector(
                "_BlurDistance",
                new Vector2(-blurDistance, blurDistance)
            );
            m_ChromAberrationMaterial.SetFloat(
                "_Luminance",
